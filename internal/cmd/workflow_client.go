...
func (a *workflowClientAdapter) Submit(input *project.InputFile, cloneBranch string, debug string, baseBranch string, items string, cleanup bool) (string, error) {
	...
	wf, err := workflow.GenerateWorkflow(a.ctx, input.Slug(), cloneBranch, projectBranch, baseBranch, items, cleanup, a.ctx.IsVerbose(), repoURL, relProjectPath)
	if err != nil {
		return "", err
	}
