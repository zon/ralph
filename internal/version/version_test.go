...
func TestVersion_ReturnsValidSemver(t *testing.T) {
	...
	assert.Equal(t, 6, major, "Major version should be 6")
	assert.Equal(t, 1, minor, "Minor version should be 1")
...
func TestVersion_PatchBumpApplied(t *testing.T) {
	...
	assert.Equal(t, "6.1.0", versionStr, "Version should be bumped to 6.1.0")
}
