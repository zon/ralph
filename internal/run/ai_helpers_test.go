...
func TestBuildChangelogPrompt(t *testing.T) {
	prompt := buildChangelogPrompt("test.txt")

	assert.NotEmpty(t, prompt, "changelog prompt should not be empty")
	assert.Contains(t, prompt, "test.txt", "prompt should reference output file")
...
func TestBuildChangelogPromptTemplateUsage(t *testing.T) {
	...
	// Check that the template is properly filled
	assert.Contains(t, prompt, "Write a concise changelog entry for the changes currently staged in git.", "prompt should contain template text")
	assert.Contains(t, prompt, "test.txt", "changelog prompt should include output file path")
	assert.Contains(t, prompt, "Do not include any extra commentary, just the changelog entry.", "prompt should contain template footer")
}
