...
func isBlocked(ctx *context.Context) (bool, error) {
	...
	blockedPath := file.Join(repoRoot, "blocked.md")
	_, err = file.Stat(blockedPath)
	if err != nil {
...
func CommitChanges(ctx *context.Context, iteration int) error {
	...
	_, reportErr := file.Stat("report.md")
	if !git.HasUncommittedChanges() && file.IsNotExist(reportErr) {
		return ErrNoChanges
	}
...
func generateChangelogIfNeeded(ctx *context.Context) error {
	...
	if _, err := file.Stat("report.md"); err == nil {
...
func getCommitMessage(iteration int) ([]byte, error) {
	...
	commitMsg, err := file.ReadFile(reportPath)
	if err != nil {
		return nil, fmt.Errorf("report.md not found: %w", err)
	}
	if err := file.Remove(reportPath); err != nil {
...
func performCommit(ctx *context.Context, commitMsg []byte, iteration int) error {
	...
	if git.HasStagedChanges() {
		...
		if err := file.Remove("report.md"); err != nil && !file.IsNotExist(err) {
			logger.Warningf("Failed to remove report.md: %v", err)
		}
		return ErrNoChanges
	}
