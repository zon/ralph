...
func (m *mockFsClient) UserHomeDir() (string, error) {
...
func (m *mockFsClient) ReadFile(name string) ([]byte, error) {
	if m.readFileFn != nil {
...
func withFsClient(f FsClient) Opt {
	return func(d *deps) {
		d.fsClient = f
	}
}
...
func newCmd(opts ...Opt) *Cmd {
	...
	return New(d.configLoader, d.fsClient, d.k8sClient, d.log)
}
...
func TestRun_Success(t *testing.T) {
	...
	k8sClient := &mockK8sClient{
		createOrUpdateSecretFn: func(ctx context.Context, name, namespace, kubeContext string, data map[string]string) error {
			createdSecret = true
			require.Equal(t, "opencode-credentials", name)
			require.Equal(t, "my-namespace", namespace)
			require.Equal(t, "my-cluster", kubeContext)
			require.Equal(t, `{"token": "test-token"}`, data["auth.json"])
			return nil
		},
	}
	...
	require.Contains(t, loggedInfo, "Configuring OpenCode credentials for Ralph remote execution...")
	require.Contains(t, loggedSuccess, "OpenCode credentials read successfully")
}
...
func TestRun_ReadCredentialsHomeDirFailure(t *testing.T) {
	fs := &mockFsClient{
		userHomeDirFn: func() (string, error) {
			return "", errMockFailure
		},
	}
	cmd := newCmd(withFsClient(fs))
	err := cmd.Run(context.Background(), "", "")
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to get user home directory")
}

func TestRun_ReadCredentialsFileNotFound(t *testing.T) {
	fs := &mockFsClient{
		readFileFn: func(name string) ([]byte, error) {
	...
	err := cmd.Run(context.Background(), "", "")
	require.Error(t, err)
	require.Contains(t, err.Error(), "OpenCode auth.json not found")
}

func TestRun_ReadCredentialsReadFailure(t *testing.T) {
	fs := &mockFsClient{
		readFileFn: func(name string) ([]byte, error) {
			return nil, errMockFailure
		},
	...
	err := cmd.Run(context.Background(), "", "")
	require.Error(t, err)
	require.Contains(t, err.Error(), "failed to read auth.json")
}

func TestRun_ReadCredentialsEmptyFile(t *testing.T) {
	fs := &mockFsClient{
		readFileFn: func(name string) ([]byte, error) {
			return []byte{}, nil
		},
	...
	err := cmd.Run(context.Background(), "", "")
	require.Error(t, err)
	require.Contains(t, err.Error(), "auth.json is empty")
}
