...
**Item** — one element of the project's resolved item array. Its 0-based index in that array identifies it, and an optional key (the scalar `slug`, `id`, or `name` field of its value) labels it. The key is only a label.

## Instructions

1. Review the incomplete items above, each labeled with its index and key.
