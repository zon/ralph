...
- [loop](ralph/loop.md) — Bounded AI iteration loop over a configured or flag-supplied list of steps, committing to a `loop-<slug>` branch and opening a pull request when the loop ends, running with `--mode` in-process locally, in a Git worktree, or in an Argo Workflow.
...
- [run-worktree](ralph/run-worktree.md) — Runs the development loop in-process in a Git worktree created in a sibling directory, leaving the current checkout untouched (the `worktree` execution mode).
- [setup](ralph/setup.md) — Confirms that git, the GitHub CLI, and OpenCode are ready to run Ralph, and, when a namespace is targeted via flag or config, confirms kubectl and the Argo CLI are installed and prepares the Kubernetes namespace for remote Argo Workflow execution.
