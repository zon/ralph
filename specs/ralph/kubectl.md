...
The commands that interact with kubectl are: `ralph list`, `ralph stop`, `ralph logs`, `ralph run` (in `remote` mode), `ralph loop` (in `remote` mode), `ralph command`, and `ralph set config`. Each SHALL accept the `--context` and `--namespace` options described below.
