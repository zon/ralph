...
The command SHALL accept `--model` to override the AI model from config and `--context` to override the Kubernetes context used for remote workflow submission. The shared `--context` and `--namespace` targeting contract is defined in [kube-options.md](kube-options.md).
